class MaquinasController < ApplicationController
  ...
  def programacion_maquinas_excel
    ...
    @ordenes_produccion = OrdenProduccion.joins(:montaje =>[:cliente,:contenedores_de_maquinas,:desarrollos_de_tintas]).where("contenedores_de_maquinas.maquina_id= ?", @maquina.id).order("ordenes_produccion.numero_de_orden")
    respond_to do |format|
      format.html
      format.xls{
