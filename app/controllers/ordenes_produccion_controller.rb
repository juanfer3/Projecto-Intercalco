class OrdenesProduccionController < ApplicationController
  ...
  @ordenes_produccion = OrdenProduccion.joins(:compromisos_de_entrega, :montaje => [:linea_producto]).paginate(page: params[:page], per_page: 20).select("DISTINCT  ordenes_produccion.id, ordenes_produccion.numero_de_orden, lineas_productos.nombre, montaje_id , entregado, sacar_de_inventario, impresion, pantalla, color, corte_material, acabado").where("compromisos_de_entrega.fecha_de_compromiso >= ? AND ordenes_produccion.entregado = ?", hoy, entregado).order("compromisos_de_entrega.fecha_de_compromiso")
