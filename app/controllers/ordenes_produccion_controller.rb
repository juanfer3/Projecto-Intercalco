class OrdenesProduccionController < ApplicationController
  ...
  @ordenes_produccion = OrdenProduccion.joins(:compromisos_de_entrega, :montaje => [:linea_producto]).paginate(page: params[:page], per_page: 20).where("compromisos_de_entrega.fecha_de_compromiso >= ? AND ordenes_produccion.entregado = ?", hoy, entregado).distinct.order("compromisos_de_entrega.fecha_de_compromiso")
